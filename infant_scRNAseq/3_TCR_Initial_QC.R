...
# Starting counts
tcr_annotated$barcode %>%
  unique() %>%
  length()
# 26052 cell barcodes and 49512 TCR contigs

# Add the PTID (sample donor) metadata from the freemuxlet-annotated RDS file to the TCR df.
...
tcr_final <- tcr_final %>% subset(!cdr3 == "None") # Remove entries with CDR3 = "None"
...
# Drop any entries with an unknown ptid (sample donor)
tcr <- tcr_final %>% drop_na(ptid)
tcr$barcode %>%
  unique() %>%
  length()
...
# Reorder the columns of the tcr DF according to the example data
...
## For each cell in the "tcr_combined" dataframe, determine whether it is Vd1, Vd3, Vg9Vd2, 
# or Other_Vd2 (Vd2 not paired with Vg9) and add this to a new column "tcr_group"
# Create a new column with the paired TCR genes
tcr_combined$TRG <- substr(tcr_combined$TCR1, 1, 5)
tcr_combined$TRD <- substr(tcr_combined$TCR2, 1, 5)
tcr_combined$paired_tcr <- paste(tcr_combined$TRD, tcr_combined$TRG, sep = "_")

# Annotate the TCR groups
tcr_vd1 <- tcr_combined %>%
  filter(TRD == "TRDV1") %>%
  mutate(tcr_group = "Vd1")
tcr_vd3 <- tcr_combined %>%
  filter(TRD == "TRDV3") %>%
  mutate(tcr_group = "Vd3")
tcr_vg9vd2 <- tcr_combined %>%
  filter(paired_tcr == "TRDV2_TRGV9") %>%
  mutate(tcr_group = "Vg9Vd2")
tcr_other_vd2 <- tcr_combined %>%
  filter(TRD == "TRDV2" & !(paired_tcr == "TRDV2_TRGV9")) %>%
  mutate(tcr_group = "Other_Vd2")
...
# Export the df containing the TCR group assignments
