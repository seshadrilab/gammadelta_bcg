...
## Please substitute the anonymized IDs in the SRA downloads for the following animal IDs:
## SEDXX2: V113
## SEDXX5: V015
## SEDXX6: DG0D
## SEDXX7: DG0H
...
# Read in the filtered barcode matrices
dg0d <- Read10X("dg0d_sample_filtered_feature_bc_matrix")
dg0h <- Read10X("dg0h_sample_filtered_feature_bc_matrix")
v015 <- Read10X("v015_sample_filtered_feature_bc_matrix")
v113 <- Read10X("v113_sample_filtered_feature_bc_matrix")

# Set the donor IDs
ptids <- c("dg0d", "dg0h", "v015", "v113")

# Record the initial counts in a table to track the number of barcodes remaining after each QC step
...
# Extract the ADT (antibody-derived tag) and HTO (hashtag oligo) matrices for each donor
...
# Only keep HTOs that appear in at least 50 cells (to prevent demux errors)
...
# Only keep cells with >2 total HTO expression (to prevent demux errors)
...
hto_tbl <- hto_tbl %>% mutate(
  Doublet_pct = Doublet / Sum,
  HTO1_pct = HTO1 / Sum,
  HTO2_pct = HTO2 / Sum,
  HTO6_pct = HTO6 / Sum,
  HTO7_pct = HTO7 / Sum,
...
# Annotate PTID (sample donor), tissue, and timepoint for each HTO
# dg0d
